...
💡 접근 방식: `recoil` 로 `client 상태` 를 처리하고 `react-query` 로 `server 상태` 를 처리하였으며, query문들에 대한 `dependencies` 들을 이용하여 최대한 간결하게 처리 및 사용하였다.
...
💡 접근 방식: 초기 화면 조차 렌더링이 안되어 하얀 화면만 나오는 팀들이 9할이였고, 이에따라 페이지 컴포넌트 별로 `code splitting` 을 통하여 초기 `rendering bundle`을 
